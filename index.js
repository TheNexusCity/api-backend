...
const _readJson = async (request) => {
  ...
  return new Promise((accept, reject) => {
    ...
    request.on('end', () =>
      accept(_jsonParse(Buffer.concat(buffers).toString('utf8')))
    );
      ...
      if (!aiPrefixLoad) {
        aiPrefixLoad = (async () => {
          const res = await fetch(
            `https://webaverse.github.io/code-ai/ai-prefix.js`
          );
...
const _makeSendRequestRewriter = (OPEN_AI_URL, engine) =>
...
openai._send_request = _makeSendRequestRewriter(
  'https://api.openai.com/v1',
  engines.gpt3
);
const openaiCodex = new OpenAI(config.openAiKey);
openaiCodex._send_request = _makeSendRequestRewriter(
  'https://api.openai.com/v1',
  engines.codex
);
const gooseAiLore = new OpenAI(config.gooseAiKey);
gooseAiLore._send_request = _makeSendRequestRewriter(
  'https://api.goose.ai/v1',
  engines.lore
);
const _openAiCodex = async (
  prompt,
  { stop, max_tokens = NaN, temperature, top_p }
) => {
      ...
      if (method === 'POST') {
        ...
        if (email && emailRegex.test(email)) {
          ...
          if (whitelisted) { */
            ...
            console.log(
              `login with email ${email} @ ${req.connection.remoteAddress} ${
                ok ? 'OK' : 'BLOCKED'
              }`
            );
...
try {
  ...
  const _handleAi = async (req, res) => {
    ...
    try {
      ...
      const o = JSON.parse(s); */
        ...
        req.method === 'POST' &&
        p === '/ai21/v1/engines/j1-large/completions'
      ) {
        _setCorsHeaders(res);
      ...
      res.end();
        ...
        req.method === 'POST' &&
        p === '/gooseai/v1/engines/gpt-neo-20b/completions'
      ) {
        _setCorsHeaders(res);
  ...
  const _req = (protocol) => (req, res) => {
    try {
      ...
      if (
        o.host === 'login.webaverse.com' ||
        o.pathname.toLowerCase() === '/login'
      ) {
